<?php
class OmcCustomersCredit extends AppModel
{
    /**
     * associations
     */
    ...
    function setUp($sheet_id,$comp_id,$omc_id){
        ...
        $total_todays_credit_sales = $OmcDailySalesProduct->sumField($sheet_id,'customers_day_sales_value');

        //Update Cumulative Credit Sales
        $total_cumulative_credit_sales = $total_todays_credit_sales + doubleval($credit_bf_prev_day);
        $omc_credit_note  = doubleval($data['omc_issued_credit_note']);

        //Update Balance On Credit Sales
        $balance_on_credit_sales  = $total_cumulative_credit_sales  - $omc_credit_note;
    ...
    function getFieldValue($sheet_id,$field){
        ...
        if($res){
            return $res['OmcCustomersCredit'][$field];
        }
        else{
            return false;
        }
    ...
    function getTableSetup(){
        $table_setup = array(
            ...
            'omc_issued_credit_note'=>array('field'=>'omc_issued_credit_note','header'=>'OMC Issued Credit Note','unit'=>'','editable'=>'yes','field_type'=>'text','validate'=>'required,numeric','format'=>'money'),
