...
<div class="span9">
    ...
    <table id="dg" title="" style="height: 400px;" toolbar="#toolbar" pagination="true" idField="id" rownumbers="true" fitColumns="true" singleSelect="true">
        <thead>
        <tr>
            <th field="name"width="50" sortable="true" editor="{type:'validatebox',options:{required:true}}">Omc Name</th>
        </tr>
        </thead>
    </table>
...
<?php
    ...
    echo $this->Html->script('scripts/omclist.js');
?>

<input type="hidden" id="grid_get_url" value="<?php echo $this->Html->url(array('controller' => 'Administration', 'action' => 'omclist/get')); ?>" />
<input type="hidden" id="grid_save_url" value="<?php echo $this->Html->url(array('controller' => 'Administration', 'action' => 'omclist/save')); ?>" />
<input type="hidden" id="grid_load_url" value="<?php echo $this->Html->url(array('controller' => 'Administration', 'action' => 'omclist/load')); ?>" />
<input type="hidden" id="grid_delete_url" value="<?php echo $this->Html->url(array('controller' => 'Administration', 'action' => 'omclist/delete')); ?>" />