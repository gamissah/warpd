var BdcUsers = {
    ...
    handleGridEvent:function (com, grid, json) {
        if (com == 'New') {
            BdcUsers.objGrid.flexBeginAdd();
        }
        else if (com == 'Edit') {
            var row = FlexObject.getSelectedRows(grid);
            BdcUsers.objGrid.flexBeginEdit(row[0]);
        }
        else if (com == 'Save') {
            BdcUsers.objGrid.flexSaveChanges();
        }
        else if (com == 'Cancel') {
            BdcUsers.objGrid.flexCancel();
        }
        else if (com == 'Delete') {
            if (FlexObject.rowSelectedCheck(BdcUsers.objGrid,grid)) {
                BdcUsers.delete_(grid);
            }
        }
        else if (com == 'Reset Password') {
            if (FlexObject.rowSelectedCheck(BdcUsers.objGrid,grid,1)) {
                var row = FlexObject.getSelectedRowIds(grid);
                BdcUsers.resetPass(row[0]);
            }
        }
    },
