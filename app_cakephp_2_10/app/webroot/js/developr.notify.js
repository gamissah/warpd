...
;(function($, window, document, undefined)
{
	...
	{
		// Timer
		element.data('notification-timeout', setTimeout(function() { element.fadeAndRemove().trigger('n_close'); }, delay));
	...
	window.notify = function(title, message, options, rotate)
	{
		...
		else
		{
			...
			var classes = ['notification'].concat(settings.classes),
				...
				closeButton = settings.closeButton ? '<span class="n_close'+(settings.showCloseOnHover ? ' show-on-parent-hover' : '')+'">✕</span>' : '',
			...
			if (settings.groupSimilar)
			{
				siblings.each(function(i)
				{
                    ...
                    //if (data.title === title.toLowerCase())// Mek Change
					{
                        //Mek Change we have to update to the current message if the titles are the same
			...
			if (settings.onClose)
			{
				element.on('n_close', settings.onClose);
			}
