...
class CepsAdminController extends CepsAppController
{
    # Controller name

    var $name = 'CepsAdmin';
    # set the model to use
    var $uses = array('User','Group','Menu','MenuGroup', 'Cep');
        ...
        if ($this->request->is('ajax')) {
            ...
            switch ($type) {
                case 'get' :
                    ...
                    $condition_array = array('User.cep_id' => $company_profile['id'], 'User.deleted' => 'n');
                ...
                case 'save' :
                    ...
                    $data['User']['cep_id'] = $company_profile['id'];
        ...
        if ($this->request->is('ajax')) {
            ...
            switch ($type) {
                case 'get' :
                    ...
                    $condition_array = array('Group.type' =>$this->Auth->user('user_type'),'Group.cep_id' => $company_profile['id'], 'Group.deleted' => 'n');
                ...
                case 'save' :
                    ...
                    $data['Group']['cep_id'] = $company_profile['id'];
    ...
    function access_control($group_id=null){
        ...
        if ($this->request->is('post')) {
            ...
            $this->MenuGroup->deleteAll(array('MenuGroup.cep_id' => $company_profile['id'],'MenuGroup.group_id' => $data['group_id']), false);
    ...
    function company()
    {
        ...
        if ($this->request->is('post')) {
            $res = $this->Cep->save($this->sanitize($this->request->data));
        ...
        $org = $this->Cep->find('first', array(
            'conditions' => array('Cep.id' => $company_profile['id']),
            'recursive' => -1
        ));
